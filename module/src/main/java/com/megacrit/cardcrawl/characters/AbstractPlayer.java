...
public abstract class AbstractPlayer extends AbstractCreature {
    ...
    public void initializeStarterDeck() {
        ...
        BaseMod.publishPostCreateStartingDeck(this.chosenClass, this.masterDeck);
    }
    ...
    public void loseGold(int goldAmount) {
        ...
        if (goldAmount > 0) {
            ...
            logger.info("NEGATIVE MONEY???", new Object[0]);
        }
                ...
                if (!tmp && this.isHoveringDropZone && this.isDraggingCard) {
                    ...
                    if (this.hoveredCard.showEvokeValue) {
                        if (this.hoveredCard.showEvokeOrbCount == 0) {
                            ...
                            int i = this.hoveredCard.showEvokeOrbCount;
                            int emptyCount = 0;
    ...
    private void updateSingleTargetInput() {
        ...
        if (this.isInKeyboardMode) {
            if (!InputActionSet.releaseCard.isJustPressed() && !CInputActionSet.cancel.isJustPressed()) {
                ...
                AbstractCard cardFromHotkey = this.hoveredCard;
                this.inSingleTargetMode = false;
                this.hoveredMonster = null;
    ...
    private void updateTargetArrowWithKeyboard(boolean autoTargetFirst) {
        ...
        if (directionIndex != 0) {
            ...
            for(AbstractMonster m : prefiltered) {
                if (m.isDying) {
            ...
            if (this.hoveredMonster == null) {
                if (directionIndex == 1) {
                    newTargetMonster = (AbstractMonster)sortedMonsters.get(0);
                } else {
                    newTargetMonster = (AbstractMonster)sortedMonsters.get(sortedMonsters.size() - 1);
                }
            } else {
                ...
                newTargetMonster = (AbstractMonster)sortedMonsters.get(newTargetIndex);
            }
    ...
    public void damage(DamageInfo info) {
        ...
        if (info.owner != null) {
            ...
            logger.info("NO OWNER, DON'T TRIGGER POWERS", new Object[0]);
        }
    ...
    public void draw(int numCards) {
        for(int i = 0; i < numCards; ++i) {
            if (this.drawPile.isEmpty()) {
                logger.info("ERROR: How did this happen? No cards in draw pile?? Player.java", new Object[0]);
            } else {
    ...
    public boolean loseRelic(String targetID) {
        if (!this.hasRelic(targetID)) {
            ...
            if (toRemove == null) {
                logger.info("WHY WAS RELIC: " + this.name + " NOT FOUND???", new Object[0]);
                return false;
            } else {
    ...
    public void reorganizeRelics() {
        logger.info("Reorganizing relics", new Object[0]);
    ...
    public boolean obtainPotion(AbstractPotion potionToObtain) {
        ...
        if (index < this.potionSlots) {
            ...
            logger.info("NOT ENOUGH POTION SLOTS", new Object[0]);
    ...
    public void evokeOrb() {
        if (!this.orbs.isEmpty() && !(this.orbs.get(0) instanceof EmptyOrbSlot)) {
            ...
            for(int var3 = 0; var3 < this.orbs.size(); ++var3) {
                ((AbstractOrb)this.orbs.get(var3)).setSlot(var3, this.maxOrbs);
            }
        }
    ...
    public void channelOrb(AbstractOrb orbToSet) {
        if (this.maxOrbs <= 0) {
            ...
            for(int plasmaCount = 0; plasmaCount < this.orbs.size(); ++plasmaCount) {
    ...
    public void increaseMaxOrbSlots(int amount, boolean playSfx) {
        if (this.maxOrbs == 10) {
            ...
            for(int var4 = 0; var4 < this.orbs.size(); ++var4) {
                ((AbstractOrb)this.orbs.get(var4)).setSlot(var4, this.maxOrbs);
            }
        }
